...
function createGraphQLSchema(config, keystone) {
  ...
  graphQLSchema = mapSchema(graphQLSchema, {
    'MapperKind.OBJECT_TYPE'(type) {
      if (config.lists[type.name] !== undefined && config.lists[type.name].fields._label_ === undefined) {
        ...
        return new GraphQLObjectType(_objectSpread2({
          fields
        }, objectTypeConfig));
      }
  ...
  graphQLSchema = mergeSchemas(_objectSpread2({
    schemas: [graphQLSchema]
  }, getAdminMetaSchema({
    keystone,
    config
...
function itemAPIForList(list, context, getArgs) {
  ...
  return {
    ...
    findMany(_ref2) {
      ...
      if (resolveFields) {
        return getItems(_objectSpread2({
          listKey,
          context,
          returnFields: resolveFields
        }, args));
...
function makeCreateContext({
  ...
  const createContext = ({
    ...
    const contextToReturn = _objectSpread2(_objectSpread2(_objectSpread2({
      schemaName: 'public'
...
function createKeystone(config, dotKeystonePath, script) {
  ...
  if (db.adapter === 'knex') {
    ...
    adapter = new MongooseAdapter(_objectSpread2({
      mongoUri: db.url
    }, db.mongooseOptions));
  } else if (db.adapter === 'prisma_postgresql') {
    adapter = new PrismaAdapter(_objectSpread2({
...
function applyIdFieldDefaults(config) {
  ...
  Object.keys(config.lists).forEach(key => {
    ...
    const fields = _objectSpread2({
      id: idField
    }, listConfig.fields);

    lists[key] = _objectSpread2(_objectSpread2({}, listConfig), {}, {
      fields
    });
  });
  return lists;

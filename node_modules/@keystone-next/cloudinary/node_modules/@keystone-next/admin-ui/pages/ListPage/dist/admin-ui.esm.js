...
function FieldSelection({
  ...
  const setNewSelectedFields = selectedFields => {
    if (isArrayEqual(selectedFields, list.initialColumns)) {
      ...
      router.push({
        query: _objectSpread2(_objectSpread2({}, router.query), {}, {
...
function Page({
  ...
  const newQuery = _objectSpread2(_objectSpread2({}, query), {}, {
    page: page
  });
  ...
  const filters = useMemo(() => {
    ...
    Object.keys(query).forEach(key => {
      ...
      if (filter && typeof val === 'string') {
        ...
        if (val !== undefined) {
          filters.push(_objectSpread2(_objectSpread2({}, filter), {}, {
            value
          }));
        }
      }
...
function useQueryParamsFromLocalStorage(listKey) {
  ...
  useEffect(() => {
    ...
    if (!hasSomeQueryParamsWhichAreAboutListPage) {
      ...
      if (parsed) {
        router.replace({
          query: _objectSpread2(_objectSpread2({}, router.query), parsed)
        });
      }
    }
  }, [localStorageKey]);

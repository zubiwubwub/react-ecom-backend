...
const appTemplate = (config, system, {
  configFileExists,
  projectAdminPath
}) => {
  const {
  ...
  for (const views of allViews) {
    viewPaths[views] = Path__default['default'].isAbsolute(views) ? Path__default['default'].relative(Path__default['default'].join(projectAdminPath, 'pages'), views) : views;
  } // -- TEMPLATE START
...
const writeAdminFiles = (config, system, configFileExists, projectAdminPath) => [...['next.config.js', 'tsconfig.json'].map(outputPath => ({
  mode: 'copy',
  ...
  src: appTemplate(config, system, {
    configFileExists,
    projectAdminPath
  })
}, {
  mode: 'write',
  src: homeTemplate(system.keystone.lists),

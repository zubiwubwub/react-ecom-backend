...
import 'graphql';
import 'crypto';
import { w as writeAdminFiles, v as viewHash } from '../../dist/index-1e82a761.esm.js';
import url from 'url';
...
const generateAdminUI = async (config, system) => {
  ...
  const projectAdminPath = Path.resolve(process.cwd(), './.keystone/admin'); // Nuke any existing files in our target directory
  ...
  const baseFiles = adminFiles.filter(x => !uniqueFiles.has(Path.normalize(x.outputPath))); // this should always exist, the user should not be able to override it.
...
const createAdminUIServer = async (ui, system, dev, sessionImplementation) => {
  ...
  const app = next({
    dev,
    dir: Path.join(process.cwd(), '.keystone', 'admin')
  });
...
export { buildAdminUI, createAdminMeta, createAdminUIServer, generateAdminUI, getAdminMetaSchema };

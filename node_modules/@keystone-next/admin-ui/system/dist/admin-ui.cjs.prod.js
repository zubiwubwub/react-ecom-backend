...
require('graphql');
require('crypto');
var templates_dist_adminUi = require('../../dist/index-4ff257f9.cjs.prod.js');
var url = require('url');
...
async function writeAdminFile(file) {
  const projectAdminPath = Path__default['default'].resolve(process.cwd(), './.keystone/admin');
...
const generateAdminUI = async (config, system) => {
  var _config$ui$getAdditio, _config$ui, _config$ui$getAdditio2;

  const projectAdminPath = Path__default['default'].resolve(process.cwd(), './.keystone/admin'); // Nuke any existing files in our target directory

  await fs__default['default'].remove(projectAdminPath); // Write out the files configured by the user

  const userPages = (_config$ui$getAdditio = (_config$ui = config.ui) === null || _config$ui === void 0 ? void 0 : (_config$ui$getAdditio2 = _config$ui.getAdditionalFiles) === null || _config$ui$getAdditio2 === void 0 ? void 0 : _config$ui$getAdditio2.map(x => x(config, system))) !== null && _config$ui$getAdditio !== void 0 ? _config$ui$getAdditio : [];
  ...
  const adminFiles = templates_dist_adminUi.writeAdminFiles(config, system, configFileExists, projectAdminPath);
  const baseFiles = adminFiles.filter(x => !uniqueFiles.has(Path__default['default'].normalize(x.outputPath))); // this should always exist, the user should not be able to override it.
  ...
  await Promise.all(baseFiles.map(writeAdminFile)); // Add files to pages/ which point to any files which exist in admin/pages
  ...
  await Promise.all(files.map(async filename => {
    ...
    await fs__default['default'].outputFile(outputFilename, `export { default } from "${path}"`);
  }));
};

const createAdminUIServer = async (ui, system, dev, sessionImplementation) => {
  var _ui$publicPages;
  ...
  const app = next__default['default']({
    dev,
    dir: Path__default['default'].join(process.cwd(), '.keystone', 'admin')
  });
